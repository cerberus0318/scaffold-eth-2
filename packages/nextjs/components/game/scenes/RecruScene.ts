export default class RecruScene extends Phaser.Scene {
  coinsCollected = 0;
  ...
  preload(): void {
    this.load.scenePlugin("AnimatedTiles", "./assets/AnimatedTiles.js", "animatedRecru", "animatedRecru");
  }
  ...
  create(data: any): void {
    ...
    // Animation
    this.animatedRecru.init(this.map);
  ...
  update(): void {
    ...
    this.animatedRecru.updateAnimatedTiles();
    if (isMobile) {
      this.moveCharacter(this.cursorKeys);
    } else {
