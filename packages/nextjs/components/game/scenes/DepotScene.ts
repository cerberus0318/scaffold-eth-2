export default class DepotScene extends Phaser.Scene {
  coinsCollected = 0;
  ...
  preload(): void {
    this.load.scenePlugin("AnimatedTiles", "./assets/AnimatedTiles.js", "animatedDepot", "animatedDepot");
  }
  ...
  create(data: any): void {
    ...
    this.map = this.make.tilemap({ key: "depotMap" });
    ...
    // Animation
    this.animatedDepot.init(this.map);
  ...
  update(): void {
    ...
    this.animatedDepot.updateAnimatedTiles();
    if (isMobile) {
      this.moveCharacter(this.cursorKeys);
    } else {
